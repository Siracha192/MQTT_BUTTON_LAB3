...
void LED_Control_Task(void *params)
{
    ...
    while (true)
    {
        if (xQueueReceive(interputQueue1, &pinNumber, portMAX_DELAY))
        {
            ...
            esp_mqtt_client_publish(mqtt_client, "/stu_192/lamp1", data, 0, 0, 0); // <-- publish ไปยัง broker
        }
    }
}
...
void LED2_Control_Task(void *params)
{
    ...
    while (true)
    {
        if (xQueueReceive(interputQueue2, &pinNumber, portMAX_DELAY))
        {
            ...
            esp_mqtt_client_publish(mqtt_client, "/stu_192/lamp2", data, 0, 0, 0); // <-- publish ไปยัง broker
        }
    }
}
